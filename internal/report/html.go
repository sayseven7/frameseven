...
<main class="content">
  <header class="hero">
    ...
    {{if .Confidential}}<div class="confidential-banner">Confidential — contains extracted sensitive data</div>{{end}}
  ...
  {{if .ExtractedData}}
  <section class="section" id="extracted">
    <div class="section-heading">
      <div><h2>Sensitive data extracted</h2><p>Dumps, credentials, and files recovered during exploitation. Treat every value as a confirmed exposure.</p></div>
    </div>
    <div class="findings">
      {{range .ExtractedData}}
      <article class="finding critical">
        <div class="finding-head">
          <div class="finding-kicker">
            {{if .Severity}}<span class="badge severity">{{.Severity}}</span>{{end}}
            {{if .Endpoint}}<span class="badge">{{.Endpoint}}</span>{{end}}
          </div>
          <h3>{{.Title}}</h3>
        </div>
        <div class="finding-body"><details open><summary>Extracted data</summary><pre>{{.Data}}</pre></details></div>
      </article>
      {{end}}
    </div>
  </section>
  ...
  <section class="section" id="false-positives">
    <div class="section-heading">
      <div><h2>Appendix: discarded false positives</h2><p>Candidates ruled out during triage and excluded from the findings above.</p></div>
    </div>
    <div class="panel"><div class="panel-body"><ul class="data-list">
      {{range .FalsePositives}}<li><strong>{{.Title}}</strong>{{if .Module}} <span class="muted">{{.Module}}</span>{{end}}{{if .Endpoint}}<br><code>{{.Endpoint}}</code>{{end}}{{if .Reason}}<br><span class="muted">{{.Reason}}</span>{{end}}</li>{{end}}
    </ul></div></div>
  </section>
  {{end}}
